...
imp_nhanes = read_csv("~/Box/Research/Research projects/SSB tax/oak_nhanes.csv")
yrs = 100
...
delta_ssb = mean(imp_nhanes$ssb_g)*.035274*.179 # ounces/person/day; convert grams to ounces with *.035274,  then  multiply by percent change
...
rowMeans(dfsub)
rowSds(dfsub)
readr::write_csv(as.data.frame(dfsub), path = "dfsuboak.csv")
...
colMeans(df)
colSds(df)
readr::write_csv(as.data.frame(df), path = "dfoak.csv")

save.image(file="SSBtaxsimoak.RData")
```